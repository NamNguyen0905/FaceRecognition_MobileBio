...
# First feature level using distance by Euclidean
...
# Second feature level using distance by Minkowski with order 3
def distances2(points):
    ...
    for i in range(points.shape[0]):
        for j in range(points.shape[0]):
            p1 = points[i,:]
            p2 = points[j,:]      
            dist.append( ((abs(p1[0] - p2[0])) ** 3 + (abs(p1[1] - p2[1])) ** 3) ** 1/3 )
    return dist

# Taking the bounding box for the face detecting
...
# Getting the landmark but not saving picture with landmark on
